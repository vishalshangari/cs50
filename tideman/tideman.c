...
void sort_pairs(void)
{
    ...
    {
        int p1_differential = pairs[i].winner - pairs[i].loser;

        for (int j = i + 1; j < pair_count; j++)
        {
            int p2_differential = pairs[j].winner - pairs[j].loser;

            if (p1_differential > p2_differential)
            {
                a = pairs[i];
                pairs[i] = pairs[j];
                pairs[j] = a;
...
// Check if adding current pair would create a cycle
bool check_cycle(int starting_index)
{
...
bool check_incoming(int candidate)
{
    for (int i = 0; i < candidate_count; i++)
    {
